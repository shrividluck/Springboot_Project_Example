...
public class MovieResourceTest {
	...
	public void testGetMoviesByGenre() throws Exception {
		...
		String expectedResponseBody = new String(Files.readAllBytes(Paths.get("src/test/resources/movies-list-action.json")));
		String mvcResponse = new String(mockResponse.getResponse().getContentAsByteArray());
		JSONAssert.assertEquals(expectedResponseBody, mvcResponse, true);
		
		mockResponse = mockMvc.perform(get("/movies/GENRE/Comedy").accept(MediaType.APPLICATION_JSON)).andDo(print())
				.andReturn();
		assertThat(mockResponse.getResponse().getStatus()).isEqualTo(HttpStatus.OK.value());
		expectedResponseBody = new String(Files.readAllBytes(Paths.get("src/test/resources/movies-list-comedy.json")));
		mvcResponse = new String(mockResponse.getResponse().getContentAsByteArray());
		JSONAssert.assertEquals(expectedResponseBody, mvcResponse, true);
		
		mockResponse = mockMvc.perform(get("/movies/GENRE/Horror").accept(MediaType.APPLICATION_JSON)).andDo(print())
				.andReturn();
	...
	public void testGetMovieInfoByName() throws Exception {
		MvcResult mockResponse = mockMvc.perform(get("/movies/movie/t1").accept(MediaType.APPLICATION_JSON)).andDo(print())
				.andReturn();
		assertThat(mockResponse.getResponse().getStatus()).isEqualTo(HttpStatus.OK.value());

		String expectedResponseBody = new String(Files.readAllBytes(Paths.get("src/test/resources/movie-object.json")));
		String mvcResponse = new String(mockResponse.getResponse().getContentAsByteArray());
		JSONAssert.assertEquals(expectedResponseBody, mvcResponse, true);
	}
